...
- **Authentication abuse resistance**: a legitimate user mistyping their password several times in a row is not locked out; at the same time, coordinated credential-stuffing is rejected before reaching the credential check.
...
Questions the shape phase did not resolve and which should be addressed during further chain steps (`10x-tech-stack-selector`) or during implementation. Each entry: question name → context → who should resolve it.
