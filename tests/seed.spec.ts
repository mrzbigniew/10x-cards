...
// Risk #1: AI generation returns valid proposals from real input
// Risk #2: deck save is atomic — deck appears on dashboard after save
test('AI generation produces proposals and saved deck appears on dashboard', async ({ page }) => {
    const text = `
    ...
    const saveResponsePromise = page.waitForResponse('**/api/decks');
